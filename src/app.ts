...
app.post("/polls", (req: Request, res: Response) => {
  ...
  let resolvedAccent = DEFAULT_POLL_ACCENT_COLOR;
  if (accentColor !== undefined) {
    if (!isPollAccentColor(accentColor)) {
      res.status(400).json({
        error: `accentColor must be one of: ${POLL_ACCENT_COLORS.join(", ")}`,
      });
      return;
    }
  ...
  const poll = store.createPoll(question.trim(), trimmedOptions, resolvedAccent);
